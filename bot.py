...
TRADE_PCT  = 0.15     # 每次用15%余额
BB_PERIOD  = 10       # 布林带周期
BB_STD     = 1.8      # 布林带宽度
RSI_PERIOD = 7        # RSI 周期
RSI_BUY    = 40       # RSI 低于40才买
RSI_SELL   = 60       # RSI 高于60才卖
...
MIN_TRADE_GAP = 600   # 同一币对最少间隔10分钟才能再次交易
...
def run():
    ...
    log.info(f"Need {needed} bars = ~{needed * INTERVAL // 60} minutes to start")
    while True:
        for pair in PAIRS:
